...
export default function DynamicListView({ moduleName, dataSource = [] }) {
  ...
  const { fields, loading, error } = useDropdownFields(moduleName);
    ...
    <div className="antd dynamic-list-view">
      <Card>
        <Flex align="center" justify="space-between">
          <Tabs
            activeKey={activeTab}
            items={[{ key: 'all', label: tabLabel(moduleName, filteredData.length) }]}
            onChange={(key) => {
              setActiveTab(key);
      ...
      {error && (
        <Alert
          type="error"
          message="Could not load list fields"
          description={error}
          showIcon
        />
      )}
