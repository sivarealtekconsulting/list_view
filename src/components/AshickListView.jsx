import { Ashick_Mock_Data } from '../data/ashickMockData'
...
function getValueFromRecord(record, data) {
...
function filterFunction(record, row) {
    const value = getValueFromRecord(record, row.field).trim();

    if (row.operator === 'isEmpty') {
        return value.length === 0;
    }

    if (row.operator === 'notEmpty') {
        return value.length > 0;
    }
    ...
    return row.values.some((SelectedValue) => (
        normalizedValue.includes(String(SelectedValue).toLowerCase())
    ))
}
export default function AshickListView() {
    ...
    const filtered = useMemo(() => {
        ...
        const searchablevalue = !q || q.length < 3
            ? Ashick_Mock_Data
            : Ashick_Mock_Data.filter(
                (j) =>
        ...
        return searchablevalue.filter((job) => (
            validFilterRows.reduce((matches, row, index) => {
                const matchedRow = filterFunction(job, row);

                if (index === 0 || row.operator === 'and') {
                    return matches && matchedRow;
                }
            ...
            render: (title, record) => (
                <Space align="start" size={6} className="job-title-cell">
                    <Space direction="vertical" size={2}>
                        <RouterLink className="job-cell-link" to={`/ask-detailedview/${record.id}`}>
                            {title}
                        </RouterLink>
    ...
    const visibleColumns = useMemo(
        () => columns.filter((columns) => visibleColumnKeys.includes(columns.visibilityKey || columns.key))
